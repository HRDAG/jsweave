...
def test_acceptance_sub():
    before_tex = get_tex("tests/data/before.tex")
    fake_json = get_json("tests/data/fake-json.json")
    res = sub(before_tex, fake_json)
    assert res == get_tex("tests/data/expected.tex")

def test_acceptance_update():
    before_tex = get_tex("tests/data/before.tex")
    updated_json = get_json("tests/data/updated-json.json")
    result = update(before_tex, updated_json)
    assert result == get_tex("tests/data/expected-updated.tex")
